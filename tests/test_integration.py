...
async def test_server_initialization_and_tools_list(mock_env_vars):
    ...
    async with client:
        ...
        assert expected_tools == actual_tools, f"Expected tools: {expected_tools}, but got: {actual_tools}"
...
async def test_search_tourism_tool_execution(mock_get_api_client, mock_api_client, mock_env_vars):
    """
...
@pytest.mark.asyncio 
@patch('mcp_tourism.server.get_api_client')
async def test_tool_with_validation_error(mock_get_api_client, mock_api_client, mock_env_vars):
    """
